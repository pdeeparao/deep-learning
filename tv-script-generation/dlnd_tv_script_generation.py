...
def build_nn(cell, rnn_size, input_data, vocab_size):
    ...
    outputs,state = build_rnn(cell, embeddings)
...
def get_batches(int_text, batch_size, seq_length):
    ...
    batches = np.zeros(shape = (num_of_batches, 2, batch_size, seq_length), dtype = np.int)
    print("Batches shape - (num_of_batches, 2, batch_size, window_length):", batches.shape)
    for batch_num in range(0, num_of_batches, 1):
        start_idx = batch_num*seq_length
        for batch in range(0, batch_size, 1):
                batches[batch_num][0][batch] = int_text[start_idx:start_idx+seq_length]
                #Update the target for this batch
                start_idx = start_idx+1
                batches[batch_num][1][batch] = int_text[start_idx:start_idx+seq_length]
    #Check if the batch matches the test data
    #print(batches[0][0][2])
    return batches
